...
void* producer(void *arg){
    ...
    for(i = 0; i <loops; i++){
         ...
         if (count == 1) //We can't put anymore items in our buffer, therefore we need to wait for other thread to get()
             pthread_cond_wait(&empty, &mutex) //Pass the address of the mutex so we can let other thread control the lock
...
void*consumer(void*arg){
   ...
   while (1){
       ...
       if (count == 0) //We pass in the address of the lock b/c we are going to release for the other thread.
           pthread_cond_wait(&fill, &mutex); //we can only get an item if there is something in the container
       int tmp = get();   
